...
float ssHash(float2 p){return fract(sin(dot(p,float2(127.1,311.7)))*43758.5453);}

fragment float4 sandSimulationFragment(
    ...
    for(int i = 0; i < 5; i++){
        ...
        dropY = 1.0 - dropY;
        float d = length(uv - float2(dropX, dropY));
        falling += smoothstep(0.01, 0.0, d);
    }
    float3 sandCol = float3(0.76, 0.60, 0.35);
    float3 darkSand = float3(0.55, 0.42, 0.25);
    float grain = ssHash(cell*7.0)*0.3;
    float3 col = float3(0.1, 0.12, 0.2);
    if(uv.y < sandHeight){
        col = mix(darkSand, sandCol, 0.5+grain*0.5);
        float depth = sandHeight - uv.y;
        col *= 0.8 + 0.2*exp(-depth*5.0);
    }
    col += float3(0.8, 0.7, 0.4)*falling;
