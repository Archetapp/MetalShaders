...
float snowglobeHash(float2 p) {
    return fract(sin(dot(p, float2(127.1, 311.7))) * 43758.5453);
}

float snowglobeNoise(float2 p) {
    float2 i = floor(p);
    float2 f = fract(p);
    f = f * f * (3.0 - 2.0 * f);
    float a = snowglobeHash(i);
    float b = snowglobeHash(i + float2(1.0, 0.0));
    float c = snowglobeHash(i + float2(0.0, 1.0));
    float d = snowglobeHash(i + float2(1.0, 1.0));
    return mix(mix(a, b, f.x), mix(c, d, f.x), f.y);
}

float snowglobeSdBox(float2 p, float2 b) {
    float2 d = abs(p) - b;
    return length(max(d, 0.0)) + min(max(d.x, d.y), 0.0);
}

fragment float4 snowglobeFragment(
    ...
    float3 bgCol = mix(float3(0.15, 0.1, 0.08), float3(0.08, 0.05, 0.04), uv.y);

    float2 globeCenter = float2(0.0, 0.05);
    float globeRadius = 0.35;
    float distToGlobe = length(p - globeCenter);

    float2 baseCenter = float2(0.0, globeCenter.y - globeRadius - 0.02);
    float baseWidth = 0.25;
    float baseHeight = 0.08;

    float baseDist = snowglobeSdBox(p - baseCenter, float2(baseWidth, baseHeight));
    float baseMask = smoothstep(0.005, 0.0, baseDist);
    float3 baseColor = mix(float3(0.3, 0.2, 0.1), float3(0.5, 0.35, 0.2), (p.y - baseCenter.y + baseHeight) / (baseHeight * 2.0));
    baseColor += float3(0.1) * pow(max(1.0 - abs(p.x) / baseWidth, 0.0), 4.0);

    float3 col = bgCol;
    col = mix(col, baseColor, baseMask);

    if (distToGlobe < globeRadius) {
        float2 localP = p - globeCenter;

        float3 insideBg = mix(float3(0.1, 0.15, 0.25), float3(0.05, 0.08, 0.15), (localP.y + globeRadius) / (globeRadius * 2.0));

        float groundY = -globeRadius * 0.6;
        float groundNoise = snowglobeNoise(float2(localP.x * 20.0, 0.0)) * 0.02;
        float ground = smoothstep(groundY + groundNoise + 0.01, groundY + groundNoise - 0.01, localP.y);
        float3 groundCol = float3(0.85, 0.9, 0.95);
        insideBg = mix(insideBg, groundCol, ground);

        float2 treePos = float2(0.0, groundY + 0.06);
        float trunk = step(abs(localP.x - treePos.x), 0.008) * step(treePos.y - 0.06, localP.y) * step(localP.y, treePos.y + 0.02);
        insideBg = mix(insideBg, float3(0.3, 0.2, 0.1), trunk);
        ...
        for (int i = 0; i < 3; i++) {
            ...
            float3 treeColor = mix(float3(0.1, 0.4, 0.15), float3(0.15, 0.5, 0.2), layer / 2.0);
            float snowOnTree = smoothstep(treeY + treeH - 0.01, treeY + treeH, localP.y) * treeMask;
            treeColor = mix(treeColor, float3(0.9, 0.95, 1.0), snowOnTree * 0.5);
        ...
        for (int i = 0; i < 80; i++) {
            ...
            float age = fmod(iTime + snowglobeHash(seed) * settleTime, settleTime);
            float settled = smoothstep(settleTime * 0.7, settleTime, age);

            float2 startPos = float2(
                (snowglobeHash(seed) - 0.5) * globeRadius * 1.6,
                (snowglobeHash(seed * 2.0) - 0.5) * globeRadius * 1.6
            );

            float fallProgress = age / settleTime;
            float2 snowPos = startPos;
            ...
            float snowSize = 0.003 + snowglobeHash(seed * 3.0) * 0.004;
        ...
        insideBg += float3(0.9, 0.95, 1.0) * min(snowAccum, 1.0);
        ...
        float3 refractColor = float3(0.7, 0.8, 1.0);
        insideBg = mix(insideBg, refractColor, fresnel * 0.4);
        insideBg += float3(0.3, 0.35, 0.4) * glassEdge;

        float2 localPN = localP;
        float specAngle = atan2(localPN.y, localPN.x);
        ...
        insideBg += float3(1.0) * (spec1 * 0.4 + spec2 * 0.2);

        float globeMask = smoothstep(globeRadius + 0.003, globeRadius - 0.003, distToGlobe);
        col = mix(col, insideBg, globeMask);
    }

    float shadowDist = length(float2(p.x * 1.2, p.y - (globeCenter.y - globeRadius - 0.12)));
