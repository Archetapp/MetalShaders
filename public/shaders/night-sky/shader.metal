...
float nightSkyHash(float2 p){return fract(sin(dot(p,float2(127.1,311.7)))*43758.5453);}
float nightSkyNoise(float2 p){
    float2 i=floor(p);float2 f=fract(p);f=f*f*(3.0-2.0*f);
    return mix(mix(nightSkyHash(i),nightSkyHash(i+float2(1,0)),f.x),mix(nightSkyHash(i+float2(0,1)),nightSkyHash(i+float2(1,1)),f.x),f.y);
}
...
fragment float4 nightSkyFragment(VertexOut in[[stage_in]],constant float&iTime[[buffer(0)]],constant float2&iResolution[[buffer(1)]]){
    ...
    milky=nightSkyNoise((muv+float2(t,0))*float2(3.0,8.0))*band;
    milky+=nightSkyNoise((muv+float2(t*0.5,0))*float2(6.0,15.0))*band*0.5;
    ...
    for(float layer=0.0;layer<3.0;layer++){
        ...
        float h=nightSkyHash(starUV+layer*100.0);
        if(h>0.97){
            ...
            float3 starCol=mix(float3(0.8,0.85,1.0),float3(1.0,0.9,0.7),nightSkyHash(starUV*3.0));
            col+=starCol*star*0.8;
        }
    }
    float shootStar=fract(t*2.0);
    if(shootStar<0.1){
        float2 ssStart=float2(0.3+nightSkyHash(float2(floor(t*2.0)))*0.5,0.7+nightSkyHash(float2(floor(t*2.0)+1.0))*0.2);
