...
float osmNoise(float2 p){float2 i=floor(p);float2 f=fract(p);f=f*f*(3.0-2.0*f);
...
fragment float4 oilSlickMaterialFragment(
    ...
    float wave = osmNoise(uv*3.0+t*0.2)*0.5+osmNoise(uv*7.0-t*0.3)*0.3;
    water += float3(0.0, 0.01, 0.03)*wave;
    float oilThickness = osmNoise(uv*2.0+t*0.1);
    oilThickness += osmNoise(uv*5.0-t*0.15)*0.5;
    oilThickness += osmNoise(uv*11.0+t*0.05)*0.25;
    ...
    rainbow *= rainbow;
    float oilMask = smoothstep(0.3, 0.5, osmNoise(uv*1.5+t*0.05));
