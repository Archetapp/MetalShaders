...
void main() {
    ...
    vec2 portalCenter = hasInput ? mouseCentered : vec2(sin(iMouseTime*0.3)*0.1, cos(iMouseTime*0.2)*0.08);
    float portalRadius = 0.2 + 0.05 * sin(iMouseTime * 0.5);
    ...
    float edgeNoise = prNoise(vec2(atan(uv.y - portalCenter.y, uv.x - portalCenter.x) * 5.0, iMouseTime * 2.0)) * 0.03;
    ...
    float shimmer = sin(atan(uv.y - portalCenter.y, uv.x - portalCenter.x) * 8.0 + iMouseTime * 3.0) * 0.5 + 0.5;
    ...
    for (int i = 0; i < 8; i++) {
        float fi = float(i);
        float angle = fi * 0.785 + iMouseTime * (1.0 + fi * 0.1);
        float r = portalRadius + sin(iMouseTime * 2.0 + fi) * 0.03;
