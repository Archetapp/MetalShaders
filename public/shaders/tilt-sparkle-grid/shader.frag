...
void main() {
    ...
    float tiltX = hasInput ? (mouseUV.x - 0.5) * 1.0 : sin(iTime * 0.7) * 0.5;
    float tiltY = hasInput ? (mouseUV.y - 0.5) * 0.8 : cos(iTime * 0.5) * 0.4;
    vec3 viewDir = normalize(vec3(tiltX, tiltY, 1.0));
    vec3 lightDir = hasInput ? normalize(vec3(tiltX * 0.8, tiltY * 0.6, 1.0)) : normalize(vec3(sin(iTime * 0.3) * 0.8, cos(iTime * 0.4) * 0.6, 1.0));
